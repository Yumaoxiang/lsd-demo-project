...
*   Liot\_SleepTimerID\_e：唤醒系统的定时器ID。
    ...
    **运行结果：**
    ...
    根据demo首先进入 LIOT\_SLEEP\_MODE\_LOW 低功耗模式，然后启动1分钟的低功耗定时器，唤醒后系统判断是否为低功耗定时器唤醒系统，然后退出休眠低功耗休眠模式。![image.png](https://alidocs.oss-cn-zhangjiakou.aliyuncs.com/res/Yvenve5DNoLmwloy/img/63712daa-ffc7-40a2-a768-ccd0ad85a5cc.png)
    
    进入LIOT\_SLEEP\_MODE\_DEEP\_LOW测试日志
    
    根据demo首先进入 LIOT\_SLEEP\_MODE\_DEEP\_LOW 低功耗模式，然后启动1分钟的低功耗定时器，唤醒后系统判断是否为低功耗定时器唤醒系统，然后退出休眠低功耗休眠模式。![image.png](https://alidocs.oss-cn-zhangjiakou.aliyuncs.com/res/Yvenve5DNoLmwloy/img/c1810350-0635-4642-9f9b-52c40aae7005.png)
